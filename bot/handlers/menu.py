...
from bot.handlers.statistics import statdetail_handler, stat_handler, statcat_handler
from bot.keyboards.menu import submenu_inline_keyboard, menu_inline_keyboard, statistics_menu_keyboard, \
...
@router.callback_query(lambda c: c.data in ["stat_sum", "stat_by_category"])
...
async def show_detail_period_menu(callback: types.CallbackQuery):
    await callback.message.edit_reply_markup(reply_markup=period_menu_keyboard("stat_detail", weeks=False, months=False))
    await callback.answer()
...
@router.callback_query(lambda c: c.data in ["stat_sum_day", "stat_sum_week", "stat_sum_month"])
async def handle_stat_sum_period(callback: CallbackQuery):
    period = callback.data.split("_")[-1]
    await callback.answer()
    await stat_handler(callback, period)

@router.callback_query(lambda c: c.data in ["stat_by_category_day", "stat_by_category_week", "stat_by_category_month"])
