...
async def test_stat_handler_success(parse_date_arg_mock, get_user_mock, get_context_mock, get_async_session_mock):
    # Мокаем парсинг периода
    parse_date_arg_mock.return_value = ("2024-01-01", "2024-01-02", "день")
    # Мокаем message и callback
    message = AsyncMock()
    callback = MagicMock()
    ...
    session.execute = AsyncMock(side_effect=[
        MagicMock(scalar=MagicMock(return_value=1000)),  # Доход
        MagicMock(scalar=MagicMock(return_value=500)),   # Расход
    ])
    ...
    # Запуск
    await statistics.stat_handler(callback, "day")
    message.answer.assert_awaited()
    assert "Доход" in message.answer.call_args[0][0]
    assert "Расход" in message.answer.call_args[0][0]
...
@patch("bot.handlers.statistics.get_user", new_callable=AsyncMock)
@patch("bot.handlers.statistics.parse_date_arg")
async def test_stat_handler_no_user(parse_date_arg_mock, get_user_mock, get_context_mock, get_async_session_mock):
    parse_date_arg_mock.return_value = ("2024-01-01", "2024-01-02", "день")
    message = AsyncMock()
    callback = MagicMock()
    callback.message = message
    ...
    await statistics.stat_handler(callback, "day")
    message.reply.assert_awaited_with("Пользователь не найден.")

@pytest.mark.asyncio
@patch("bot.handlers.statistics.parse_date_arg")
async def test_stat_handler_invalid_period(parse_date_arg_mock):
    parse_date_arg_mock.return_value = None
    message = AsyncMock()
    callback = MagicMock()
    callback.message = message
    await statistics.stat_handler(callback, "badperiod")
